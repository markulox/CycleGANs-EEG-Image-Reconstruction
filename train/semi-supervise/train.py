...
for epch in range(EPCH_START, EPCH_END + 1):
    ...
    batch_j2 = []
    batch_j3 = []
    for i, (y_p, l_real_p, x_p) in enumerate(dat_loader_paired):
        ...
        if SAMPLE_INTERVAL != -1 and epch % SAMPLE_INTERVAL == 0 and i + 1 == len(dat_loader_paired):
            sample_images(epch)
        ...
        if CHCK_PNT_INTERVAL != -1 and epch % CHCK_PNT_INTERVAL == 0 and i + 1 == len(
                dat_loader_paired) and not EXPORT_DISABLE:
