...
class ConvBlockInv4(nn.Module):
    def __init__(self, stride_tuple, channel_tuple):
        super(ConvBlockInv4, self).__init__()
        assert len(stride_tuple) == 3, "Stride tuple should contains 3 elements"
        assert len(channel_tuple) == 3, "Channel tuple should contains 3 elements"
        self.conv_block01 = nn.Sequential(
            nn.Conv2d(kernel_size=(3, 3), in_channels=channel_tuple[0], out_channels=channel_tuple[1], bias=False,
                      stride=stride_tuple[0]),
            nn.BatchNorm2d(num_features=channel_tuple[1], eps=0.001, momentum=0.1, affine=True),
            nn.ReLU(),
            nn.Conv2d(kernel_size=(3, 3), in_channels=channel_tuple[1], out_channels=channel_tuple[1], bias=False,
                      stride=stride_tuple[1]),
            nn.BatchNorm2d(num_features=channel_tuple[1], eps=0.001, momentum=0.1, affine=True),
            nn.ReLU(),
            nn.Conv2d(kernel_size=(3, 3), in_channels=channel_tuple[1], out_channels=channel_tuple[2], bias=False,
                      stride=stride_tuple[2]),
            nn.BatchNorm2d(num_features=channel_tuple[2], eps=0.001, momentum=0.1, affine=True),
            nn.ReLU(),
        )

    def forward(self, x):
        return self.conv_block01(x)


class DeconvBlockInv4(nn.Module):
    def __init__(self, stride_tuple, channel_tuple):
        super(DeconvBlockInv4, self).__init__()
        assert len(stride_tuple) == 3, "Stride tuple should contains 3 elements"
        assert len(channel_tuple) == 3, "Channel tuple should contains 3 elements"
        self.conv_block01 = nn.Sequential(
            nn.ConvTranspose2d(kernel_size=(3, 3), in_channels=channel_tuple[0], out_channels=channel_tuple[1],
                               bias=False,
                               stride=stride_tuple[0]),
            nn.BatchNorm2d(num_features=channel_tuple[1], eps=0.001, momentum=0.1, affine=True),
            nn.ReLU(),
            nn.ConvTranspose2d(kernel_size=(3, 3), in_channels=channel_tuple[1], out_channels=channel_tuple[1],
                               bias=False,
                               stride=stride_tuple[1]),
            nn.BatchNorm2d(num_features=channel_tuple[1], eps=0.001, momentum=0.1, affine=True),
            nn.ReLU(),
            nn.ConvTranspose2d(kernel_size=(3, 3), in_channels=channel_tuple[1], out_channels=channel_tuple[2],
                               bias=False,
                               stride=stride_tuple[2]),
            nn.BatchNorm2d(num_features=channel_tuple[2], eps=0.001, momentum=0.1, affine=True),
            nn.ReLU(),
